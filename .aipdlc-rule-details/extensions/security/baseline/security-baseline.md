...
These security rules are MANDATORY cross-cutting constraints that apply across all ai-pdlc phases. They are not optional guidance — they are hard constraints that stages MUST enforce when generating questions, producing design artifacts, generating code, and presenting completion messages.
...
4. The finding MUST be logged in `aipdlc-docs/audit.md` with the SECURITY rule ID, description, and stage context
