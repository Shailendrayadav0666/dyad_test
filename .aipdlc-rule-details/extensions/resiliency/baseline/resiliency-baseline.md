...
## Overview
These resiliency rules are MANDATORY cross-cutting constraints that apply across all ai-pdlc phases. They are derived from established cloud reliability frameworks (such as the AWS Well-Architected Reliability Pillar and 
...
4. The finding MUST be logged in `aipdlc-docs/audit.md` with the RESILIENCY rule ID, description, and stage context
...
This extension follows the ai-pdlc principle that architectural and process decisions belong to the user, not the LLM. The model MUST present the clarifying questions defined in the rules below and use the user's answers — it MUST NOT silently choose on the user's behalf. The decisions explicitly deferred to the user are:
...
How should production changes for this workload be governed? ai-pdlc will conform the design to your answer rather than inventing a process.

A) Use our existing organizational change management process — provide the name/tool (e.g., ServiceNow, Jira Change, internal CAB). ai-pdlc will reference it and ensure deployable artifacts fit that process (change records, approval gates).

B) No formal process exists yet — ai-pdlc should propose a lightweight change management process (change record + approval + rollback note) for the team to adopt.
...
A) Use our existing CI/CD pipeline — provide the tool (e.g., GitHub Actions, GitLab CI, Jenkins, CodePipeline). ai-pdlc will produce artifacts compatible with it.

B) No pipeline exists — ai-pdlc should propose a CI/CD pipeline definition appropriate to the chosen IaC and runtime.
...
A) Use our existing DR testing / game day / chaos engineering practice — provide the reference. ai-pdlc will document test scenarios that fit it.

B) No practice exists — ai-pdlc should propose a DR testing schedule and chaos experiment plan for adoption.
...
A) Use our existing incident response process — provide the reference (e.g., PagerDuty runbooks, internal IR/on-call process). ai-pdlc will align alerting and runbooks to it.

B) No formal process exists — ai-pdlc should propose a lightweight incident response and Correction of Errors (COE) process for adoption.
...
These rules are cross-cutting constraints that apply to every ai-pdlc stage. At each stage:
