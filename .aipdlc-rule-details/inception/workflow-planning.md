...
- Read `## Context Project` in `aipdlc-state.md`. **IF `Use Artifacts: Yes`**, load **only** the exact `Artifact Path` recorded there (no auto-scan of the rest of `context-project/`) — it describes how the current system works, so use it as background context when planning. **IF `No`** or absent, skip.
...
Create `aipdlc-docs/inception/plans/execution-plan.md`:
...
### 🧪 SDET TRACK (parallel — SDET-initiated, NOT planned or executed by this workflow)
- Build and Test — run per story by SDET with **`/sdet-implement`**, in parallel with development
- SDET Sign-off — run by SDET with **`sdet-list-work`** on the epic branch once story PRs merge
  - **Rationale**: Build and Test is not a Construction stage at epic or story level; it is not scheduled here and is never auto-run
...
### 🧪 SDET TRACK (parallel, SDET-initiated — not scheduled here)
- Build and Test per story — `/sdet-implement`
...
> **📋 <u>**REVIEW REQUIRED:**</u>**  
> Please examine the execution plan at: `aipdlc-docs/inception/plans/execution-plan.md`
