...
**Error**: Existing `aipdlc-state.md` is corrupted
...
- **Solution**: Implement a currently ready story first (see the Doability Gate in `workflows/dev-implement.md` and `construction/story-selection.md`)
...
**Scenario**: `aipdlc-state.md` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: `aipdlc-state.md.backup`
...
3. Reset stage status in `aipdlc-state.md`
...
3. Mark stage as "SKIPPED" in `aipdlc-state.md`
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in aipdlc-state.md
...
**Error**: aipdlc-state.md shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in aipdlc-state.md
...
1. **Always validate state**: Check aipdlc-state.md matches actual artifacts
